...
/*   Updated: 2019/04/30 03:54:25 by srobert-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fdf	*fdf_init(t_map *map)
{
	...
	if (!(fdf = (t_fdf*)ft_memalloc(sizeof(t_fdf))))
		next_u_say(GOVNO_FDF_INIT);
	if (!(fdf->mlx = mlx_init()))
		next_u_say(GOVNO_FDF_INIT);
	if (!(fdf->win = mlx_new_window(fdf->mlx, WIDTH, HEIGHT, "YobaniyFDF")))
		next_u_say(GOVNO_FDF_INIT);
